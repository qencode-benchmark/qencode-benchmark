...
# Located rather than assumed: this file no longer sits two levels under the repo
# root, and once installed from a wheel there may be no repo at all. See _paths.
from qencode._paths import repo_root as _repo_root, data_file as _data_file, \
    default_out_dir as _default_out_dir

_ROOT = _repo_root()
REPO = _ROOT if _ROOT is not None else Path(__file__).resolve().parents[3]
...
def load_molecule(name: str) -> dict:
    catalog_path = _data_file("molecules_v4.json") or (REPO / "molecules_v4.json")
    if not catalog_path.exists():
        # Fall back to v3 catalog for molecules not yet in v4
        catalog_path = _data_file("molecules_v3.json") or (REPO / "molecules_v3.json")
...
def build_pl_hamiltonian(symbols, coords_bohr, basis, mapping,
    ...
    if canon_mapping == "parity" or use_of_bridge:
        ...
        from qencode.pipeline import of_bridge
...
def _required_versions() -> dict:
    ...
    req = {}
    path = _data_file("requirements-v4.txt") or (REPO / "requirements-v4.txt")
    try:
