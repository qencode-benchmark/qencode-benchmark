...
def main():
    ...
    units = "angstrom"
    atoms = [
        {"element": "Li", "xyz": [0.0, 0.0, 0.0]},
        {"element": "H", "xyz": [0.0, 0.0, 1.6]},
    ]
    charge = 0
    multiplicity = 1
    ...
    settings = {
        ...
        "reduction": {"freeze_core": False,
        "active_space": {"enabled": True, "num_electrons": 2, "num_spatial_orbitals": 2}},
    ...
    atom_str = "Li 0.0 0.0 0.0; H 0.0 0.0 1.6"
...
# --- Active space reduction (v1 preset for LiH) ---
    use_active_space = True  # set to False if you want the full 12-qubit version
...
# - RY on all qubits
# - RZ on all qubits
# - CX chain entanglement: 0->1->2->...->n-1
#
# This is a "template" because it contains parameters (theta).

    n = qubit_op.num_qubits
    theta = ParameterVector("theta", length=2 * n)  # 2 params per qubit for 1 layer

    ansatz_circuit = QuantumCircuit(n)
# Single-qubit rotations
    for i in range(n):
        ansatz_circuit.ry(theta[i], i)
    for i in range(n):
        ansatz_circuit.rz(theta[n + i], i)
